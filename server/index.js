...
app.post('/api/admin/login', async (req, res) => {
  ...
  if (phone === ADMIN_PHONE && password === ADMIN_PWD) {
    adminTokenExpire = Date.now() + 3600000 // 1小时有效
    res.json({ code: 0, data: { token: ADMIN_TOKEN, expire: adminTokenExpire } })
  } else {
    res.json({ code: -1, msg: '密码错误或账号不存在' })
  }
})

app.put('/api/admin/recipes/:id/cover', async (req, res) => {
  const { token, cover_img } = req.body || {}
  if (token !== ADMIN_TOKEN || Date.now() > adminTokenExpire) {
    return res.json({ code: -1, msg: '未授权，请重新登录' })
  }
  try {
    await pool.query('UPDATE recipes SET cover_img=? WHERE id=?', [cover_img || '', req.params.id])
    res.json({ code: 0, msg: 'ok' })
  } catch (e) { res.json({ code: -1, msg: e.message }) }
})
